...
/** Loads the Meta Pixel (if NEXT_PUBLIC_META_PIXEL_ID is set) and fires PageView on every route change. Never runs on /admin. */
export function MetaPixel() {
  const pathname = usePathname();
  const skip = !PIXEL_ID || pathname.startsWith("/admin");

  useEffect(() => {
    if (!skip) trackPixel("PageView");
  }, [pathname, skip]);
